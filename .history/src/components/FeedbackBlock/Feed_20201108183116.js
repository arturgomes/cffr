...
export default class Feed extends Component {
  state = {
    ...
    active_step: 0,
    skipped: new Set(),
    questions: backFromAPI.data.questions,
    steps: [
      // '',
      '1',
      '2',
  ...
  componentDidMount = () => {
    this.setState({
      questions: backFromAPI.data.questions,
      opening: backFromAPI.data.opening
    })
  ...
  handleComment = async (answer) => {
    ...
    await api.post(`/feed/${this.props.fid}/c`, {
      answers: {
  ...
  handleNext = () => {
    ...
    if (this.isStepSkipped(this.state.active_step)) {
      newSkipped = new Set(newSkipped.values());
      newSkipped.delete(this.state.active_step);
    }
    this.setState({
      active_step: (prevActiveStep) => prevActiveStep + 1,
      skipped: newSkipped
    })
  };
  ...
  handleSkip = () => {
    if (!this.isStepOptional(this.state.active_step)) {
    ...
    this.setState({
      ...
      skipped: (prevSkipped) => {
        ...
        newSkipped.add(this.state.active_step);
        return newSkipped;
      }
    })
    ...
    return (
      <BasicLayout noavatar>
        <div className={classes.root}>
          {this.state.opening}
          {this.state.active_step === this.state.steps.length ?
            (
              <React.Fragment>
                <Conclusion />
              </React.Fragment>
            ...
            (
              <React.Fragment>
                { this.state.active_step === this.state.steps.length - 1 ? (
                  <QualitativeQuestion
                    quest={this.state.questions[1]}
                ...
                <div className="">
                  ...
                  <Button
                    ...
                    className=""
                  > Voltar </Button>
                  {this.isStepOptional(this.state.active_step) && (
                    <Button
                      color="inherit"
                  ...
                  <Button onClick={this.handleNext}>
                    {this.state.active_step === this.state.steps.length - 1 ? 'Concluir' : 'Próximo'}
                  </Button>
                </div>
              </React.Fragment>
            )
          }
          <Stepper active_step={this.state.active_step}>
