...
export default function FeedbackBlock(props) {
  ...
  const [nps_value, seetNPS_value] = useState(null);
  ...
  useEffect(() => {
    ...
    //   })
    setQualitative(backFromAPI.questions[1]);
    setQuantitative(backFromAPI.questions[0]);
    setOpening(backFromAPI.opening);
    console.log(opening, backFromAPI.questions[1], quantitative)
  }, [])

  const handleNPS = async (answer) => {
    if (nps_value === null) {
      seetNPS_value(answer);
    }
  }
