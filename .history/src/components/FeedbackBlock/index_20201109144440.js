...
export default function FeedbackBlock(props) {
  ...
  const [feedid, setFeedid] = useState(props.fid);
  ...
  const kindOfQuestion = () => {
    if (nps_value >= 8) {
      return "Que legal! O que mais você gostou no atendimento?"
    }
    else if (nps_value >= 7) {
      return "O que poderíamos fazer para tornar sua experiência como consumidor ainda melhor?"
    }
    else {
      return "O que você acha que faltou hoje no atendimento a você?"
    }
  }
